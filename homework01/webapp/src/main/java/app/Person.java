...
public class Person {
	...
	public boolean validateFMLNamePart(String fml_name_part, boolean empty_allowed)
	{
	    if (empty_allowed)
	    {
	    	Matcher matcher = Pattern.compile("[\\D-]{0,150}").matcher(fml_name_part);
	    	return matcher.matches();
	    }
	    else
	    {
	    	Matcher matcher = Pattern.compile("[\\D-]{1,150}").matcher(fml_name_part);
	    	return matcher.matches();
	    }
	    
	}
	public boolean validateNumber(String p,app.Person d){
		Matcher matcher = Pattern.compile("[\\d\\-*\\#*\\+*]{2,50}").matcher(p);
		return matcher.matches();
	}

    public void deleteNumber(String p) {
	  phones.remove(p);
    }
	...
	public void setPhones(String phonesID,String phones)

	{
	this.phones.put(phonesID, phones);
	}
	// Геттеры и сеттеры
	// --------------------------------------
